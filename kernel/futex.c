				  ...
				  struct task_struct *new_owner)
{
	...
	if (old_owner) {
		raw_spin_lock_irqsave(&old_owner->pi_lock, flags);
		/*
		...
		if (pi_state->owner == NULL) {
			raw_spin_unlock_irqrestore(&old_owner->pi_lock, flags);
			return;
		}
	...
	if (new_owner) {
		raw_spin_lock_irqsave(&new_owner->pi_lock, flags);
		WARN_ON(!list_empty(&pi_state->list));
		list_add(&pi_state->list, &new_owner->pi_state_list);
		pi_state->owner = new_owner;
		raw_spin_unlock_irqrestore(&new_owner->pi_lock, flags);
	}
}
