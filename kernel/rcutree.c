	...
	if (rcu_gp_in_progress(rsp)) {
		if (ACCESS_ONCE(rnp->qsmask) & rdp->grpmask) {
			if (ULONG_CMP_GE(j, js)) {
				...
				print_cpu_stall(rsp);
			} else if (ULONG_CMP_GE(j, js - 4)) {
				struct task_struct *t;
				unsigned long flags;
				int prio;
				/*
				 * Four more jiffies to go, see if
				 * we can temporarily boost.
				 */
				t = __this_cpu_read(rcu_cpu_kthread_task);
				prio = ACCESS_ONCE(current->prio) - 1;
				if (prio < 0)
					prio = 0;
				raw_spin_lock_irqsave(&t->pi_lock, flags);
				rt_mutex_setprio(t, prio);
	...
	__releases(rcu_get_root(rsp)->lock)
{
	...
	swake_up(&rsp->gp_wq);  /* Memory barrier implied by wake_up() path. */
}
		...
		*workp = 0;
		...
		if (*workp == 0) {
			...
			*statusp = RCU_KTHREAD_WAITING;
			...
			/* Put back the priority */
			raw_spin_lock_irq(&current->pi_lock);
			prio = rt_mutex_getprio(current);
