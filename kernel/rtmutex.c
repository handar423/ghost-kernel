 ...
 *
 * lock->wait_lock must be held by the caller and interrupts disabled
 */
static int __sched
		  ...
		  struct ww_acquire_ctx *ww_ctx)
{
	...
	ret = rt_mutex_slowlock_locked(lock, state, timeout, detect_deadlock, ww_ctx,
				       &waiter);
