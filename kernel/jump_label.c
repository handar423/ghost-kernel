...
int jump_label_update_delay __read_mostly;
	...
	for (; (entry < stop) &&
		...
		if (entry->code) {
			if (kernel_text_address(entry->code)) {
				if (unlikely(jump_label_update_delay)) {
					if (jump_label_update_delay < 1000)
						udelay(jump_label_update_delay);
					else
						usleep_range(jump_label_update_delay, jump_label_update_delay);
				}
