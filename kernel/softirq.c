...
void softirq_check_pending_idle(void)
{
	...
	if (warnpending) {
		print_softirq_pending("NOHZ: local_softirq_pending",
			smp_processor_id(), warnpending);
		rate_limit++;
	}
}
# else
...
void softirq_check_pending_idle(void)
{
	...
	if (rate_limit < 10 && !in_softirq() &&
			(local_softirq_pending() & SOFTIRQ_STOP_IDLE_MASK)) {
		print_softirq_pending("NOHZ: local_softirq_pending",
                        smp_processor_id(), local_softirq_pending());
		rate_limit++;
	}
}
# endif
...
/*
 * String size for printing all softirq names, including a white space
 * between each softirq name and '\0'. Currently:
 * "HI TIMER NET_TX NET_RX BLOCK BLOCK_IOPOLL TASKLET SCHED HRTIMER RCU\0"
 */
#define MAX_SOFTIRQS_PENDING_STR 68

void print_softirq_pending(const char *prefix, int cpu, __u32 pending)
{
	int i;
	int cursor = 0;
	char pending_str[MAX_SOFTIRQS_PENDING_STR];

	for (i = 0; i < NR_SOFTIRQS; i++) {
		if (pending & (1 << i))
			cursor += snprintf(pending_str + cursor,
					   MAX_SOFTIRQS_PENDING_STR - cursor,
					   "%s ", softirq_to_name[i]);
	}

	if (cursor)
		pending_str[cursor-1] = '\0';
	else
		snprintf(pending_str, MAX_SOFTIRQS_PENDING_STR, "none");

	if (prefix)
		pr_warn("CPU%d: %s: %s\n", cpu, prefix, pending_str);
	else
		pr_warn("CPU%d: softirq_pending: %s\n", cpu, pending_str);
