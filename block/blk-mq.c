...
static struct work_struct blk_work;
static DEFINE_SPINLOCK(blk_work_lock);
static struct request *work_rq;

static void blk_work_func(struct work_struct *work)
{
	work_rq->q->softirq_done_fn(work_rq);
}
#else
...
#ifdef CONFIG_PREEMPT_RT_FULL
		/* No need to stay on this CPU */
		put_cpu_light();
		spin_lock(&blk_work_lock);
		work_rq = rq;
		INIT_WORK(&blk_work, blk_work_func);
		schedule_work_on(cpu, &blk_work);
		flush_work(&blk_work);
		spin_unlock(&blk_work_lock);
		get_cpu_light();
#else
