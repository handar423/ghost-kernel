...
extern void wake_up_q_sleeper(struct wake_q_head *head);
extern void __wake_up_q(struct wake_q_head *head, bool sleeper);
extern int wake_up_lock_sleeper(struct task_struct *p);
...
#ifdef CONFIG_PREEMPT_MATCH_TRACE
...
static inline int test_tsk_need_resched_lazy(struct task_struct *tsk) { return 0; }
static inline int need_resched_now(void)
{
	return test_thread_flag(TIF_NEED_RESCHED);
}
...
#ifdef CONFIG_PREEMPT_MATCH_TRACE
void print_preempt_match_trace(struct task_struct *tsk);
#else
static inline void print_preempt_match_trace(struct task_struct *tsk) { }
#endif
