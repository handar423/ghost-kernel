		...
		struct CommandList *c)
{
	...
	if (dev) {
		...
		if (dev->in_reset &&
			...
			swake_up_all(&h->event_sync_wait_queue);
	}
}
	...
	u8 reset_type, int reply_queue)
{
	...
	if (!rc) {
		...
		atomic_dec(&dev->commands_outstanding);
		swait_event(h->event_sync_wait_queue,
			atomic_read(&dev->commands_outstanding) <= 0 ||
			lockup_detected(h));
	}
...
static void hpsa_scan_start(struct Scsi_Host *sh)
{
	...
	while (1) {
		...
		h->scan_waiting = 1;
		spin_unlock_irqrestore(&h->scan_lock, flags);
		swait_event(h->scan_wait_queue, h->scan_finished);
...
reinit_after_soft_reset:
	...
	init_swait_queue_head(&h->scan_wait_queue);
	init_swait_queue_head(&h->event_sync_wait_queue);
